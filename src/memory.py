...
class MushroomBody(object):
    ...
    def __init_connection_weights(self):
        self._W_KCtoMBON = np.ones(self._KC_nb, dtype='float32')
    ...
    def _update_mbon_activity(self):
        self._mbon_activity = np.matmul(self._W_KCtoMBON, self._kc_spikes) / float(self._normed_KC_nb)
    ...
    def learn(self):
        self._W_KCtoMBON[self._kc_spikes == 1] = 0
